...
st.caption("Runs locally if enabled; by default, SubSentry does not call external services")

q = st.text_input(
    "Question",
    key="ask_q",
    placeholder="e.g., Why did spending spike in April? Any new periodic patterns? Summarize 2025 so far.",
)
...
if df.empty:
    ...
    currency = (
        df_year["currency"].dropna().mode().iloc[0]
        if "currency" in df_year.columns and not df_year["currency"].dropna().empty
        else "USD"
    )
    ...
    if df_sub.empty:
        ...
        # Use bars to enforce discrete month ticks (no in-between labels)
