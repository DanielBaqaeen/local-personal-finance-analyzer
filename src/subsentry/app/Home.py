...
st.markdown(
    """**Recommended flow**
1) **Statements**: import one or more monthly CSV statements. Each import is tracked with a detected period and a `YYYY-MM` label
2) **Subscriptions**: inspect detected periodic patterns, including estimated cadence, typical amount, last seen date, next expected charge, and confidence
...
- **Browser UI, local server**: Streamlit runs on your machine (usually `http://localhost:8501`)
"""
)
