...
def _shrink_payload(payload: dict) -> dict:
    ...
    if hasattr(ms, "to_dict"):
        try:
            rows = ms.tail(18).to_dict(orient="records")  # type: ignore[attr-defined]
    ...
    if isinstance(alerts, list):
        ...
        try:
            out["alerts"] = alerts.head(25).to_dict(orient="records")  # type: ignore[attr-defined]
        except Exception:
            pass

    # Subscriptions / periodic patterns summary (limit)
    subs = payload.get("subscriptions") or payload.get("periodic_patterns")
    if isinstance(subs, list):
        out["periodic_patterns"] = subs[:25]
    elif hasattr(subs, "to_dict"):
        try:
            out["periodic_patterns"] = subs.head(25).to_dict(orient="records")  # type: ignore[attr-defined]
        except Exception:
            pass
