...
def _cli_encryption_ctx(cfg, SessionFactory):
    ...
    if not passphrase or not salt:
        # locked for CLI operations; keep key None (writes will be plaintext)
        return EncryptionCtx(key=None)
...
def cmd_ui(cfg):
    # Use a subprocess to invoke Streamlit reliably across Streamlit versions.
    # Swallow Ctrl+C so Windows users don't see a scary traceback.
    import subprocess
    import sys
    from pathlib import Path
    ...
    try:
        ...
        # User stopped Streamlit with Ctrl+C
        return
